...
Two of the developed metrics, step detection and dribble detection, performed very  well, with results over 90\%. The other two (jumps and position tracking while running) showed poor results, and should be improved in the future, to have better insights about this movements.
...
Regarding the metrics identification algorithms, the algorithms with the worst results, like the jumps, or the position tracking while running, should be improved, and more advanced classification techniques could be used, like machine learning methods, as seen in the related works, in Section~\ref{sec:relatedWork}. However, this would mean that a large amount of data should be collected to provide to the machine learning classification methods.

Another approach to metrics that are hard to measure using only \gls{IMU} sensors can be the use of external infrastructures, like the \gls{RFID} system used by Kinexon, that places antennas around the field. This goes against the objectives defined in Section~\ref{sec:objectives}, but could be a overall improvement to the system.
