...
In order to tackle the drawbacks of video and \gls{GPS} solutions, Kinexon~\cite{kinexon} developed a system using a wearable sensor that can accurately track basketball players positions indoors, using Ultra-Wide Band (\gls{UWB}) technology, and claims to monitor player load and efforts measured in real-time during practices or matches, providing tactic and technic analysis~\cite{kinexonarticle}.
...
Then, the data collected by the inertial sensors will be used and analysed in order to calculate the trajectories performed by each individual player and also to provide other metrics, such as distance traveled and number of jumps. A dashboard will be developed, capable to illustrate individual metrics for each player, but also providing an aggregated view regarding the whole team performance, like attacking and defending strategies in the form of heatmaps. Finally, this system (body sensor network and dashboard) will be tested and validated.
...
Summarizing, the main goal of this dissertation is to study and analyse ways to track player positions and to measure basketball game metrics, collected from inertial sensors, and to provide that insights to coaches through an easy to understand dashboard.
...
\begin{enumerate}
    ...
    \item Development of an application that collects data from inertial sensors, analyses it and displays the insights in a dashboard
