...
\label{cha:state_art}
This chapter will start by covering the different approaches available commercially to track players in different sports, like basketball, football, baseball, tennis and american football. Then the sensors used to measure players movements and the application scenarios are presented and finally the communication technologies used to transmit that data are described and compared.
...
\subsection{Video Based Tracking System}
...
The sensor approaches usually combine the use of a positioning system like a \gls{GNSS} (Global Navigation Satellite System) or a Radio Frequency positioning system like \gls{UWB} (Ultra-Wideband) or \gls{RFID} (Radio Frequency Identification) and \gls{IMU} (Inertial Measuring Unit) sensors. An \gls{IMU} is a device that measures orientation, velocity and forces using accelerometers, gyroscopes and magnetometers.

For tracking soccer players, a high number of teams use STATSports Apex, a tracking device that includes \gls{GPS}, accelerometer, gyroscope and magnetometer, and communicates using \gls{BLE} (Bluetooth Low Energy). It is used by over 280 teams in Europe, 60 in the North America and 70 in the rest of the world~\cite{statsports_clients}. 

By using \gls{BLE}, this device can connect simultaneously to hearth rate sensor, smart watches and mobile devices, gathering all the data and becoming the center of a body network, and this data can be transferred, stored and accessed online, providing over 25 insightful metrics, like total speed or number of accelerations~\cite{statsports_apex}. 

For positioning, this system uses \gls{GPS} combined with \gls{UWB} positional beacons. For outdoor or stadium environments, \gls{GPS} is used, combined with 2 positional beacons to enhance the \gls{GPS} position. For indoor environments, at least 4 \gls{UWB} beacons are used for providing accurate positioning~\cite{statsports_mapps}.

In \gls{NBA}, wearable devices aren’t allowed in game nights. However, these sensors are used in training situations. Kinexon~\cite{kinexon} is a widely used system in basketball teams. Kinexon uses an \gls{UWB} Radio system to accurately measure the location of the players, based on the time-of-arrival of the signal sent by a beacon used by the players~\cite{ghentuwb}. 

In the National Football League (\gls{NFL}), Zebra~\cite{zebranfl} uses \gls{RFID} tags on the players shoulder pads to track and to provide analytics to coaches, players and fans.

Sensor based tracking systems are more portable and provide teams and players more freedom in changing the playing court. They are typically cheaper, making it accessible to amateur teams. These systems track load and motion metrics like the covered distance, but lack basketball specific metrics like shoots and passes. These systems also need an outside infrastructure to be able to position the players, either by \gls{GPS} or \gls{UWB} antennas around the court. 
...
\label{sec:sensors}
A sensor is a device that detects or measures a physical property, and converts that value to an electric signal, which is usually transmitted to a processing unit. The input can be something like light, heat, motion, pressure, moisture, and many other physical or chemical phenomenon. %%reference??
...
There are two main methods to measure the hearth rate: Electrocardiography (\gls{ECG}) and Photoplethysmography (\gls{PPG})~\cite{ppgreview}.
...
Figure~\ref{fig:pressure} shows the product commercialized by Moticon.
...
This subsection will present some sensors used to measure motion, in the form of acceleration, rotation, heading or position in the earth. The data sensed by these sensors can then be used to measure velocities, distances or paths.
...
There are various classes of gyroscopes, used for different needs of performance, cost and volume: mechanical gyroscopes, optical gyroscopes and \gls{MEMS} (Micro-electromechanical system) gyroscopes~\cite{gyro1}.
...
\subsubsection{Application scenarios of IMU sensors}
\label{subsub:applicationscenarios}
There are various scientifical application scenarios of pedestrian tracking and activity recognition using \gls{IMU} sensors.

Regarding pedestrian tracking Wonho Kang and Youngnam Han propose an indoor location system using smartphone sensors, using step detection, step length estimation and heading direction estimation to trace the path~\cite{smartphonepdr}.  

Carl Fischer, Poorna Sukumar and Mike Hazas propose a tutorial that implements a pedestrian tracker using a Kalman Filter and correcting the velocity using zero-velocity updates and correcting the gyroscope bias~\cite{tutorial}.

Sebastian Madgwick presents a novel orientation algorithm, that claims to be more accurate than the Kalman based algorithm~\cite{madgwick}. Both approaches use a shoe-mounted sensor.
...
SigFox is complementary with other technologies, like Bluetooth, \gls{GPS}, Cellular and Wi-Fi, unleashing the applications of this technology.
...
\subsubsection{NFC}
\label{subsubsec:nfc}
\gls{NFC} (Near Field Communication) is a very short-range (0-10cm) wireless protocol, establishing wireless connections between network appliances and consumer electronic, carrying very low amounts of data (424 Kbps).
Some examples are touching the pay terminal with the \gls{NFC}-enabled phone to authorize the payment, as shown in Figure~\ref{fig:nfc}, or connecting electronic devices, like a smartphone and a headset. \cite{nfc}
...
\gls{BLE} is the technology that meets most of these requirements and is the technology that will be used in the following phases of this work. Besides meeting the requirements, this choice has in consideration other factors: Bluetooth is a technology that I had worked before and the devices available in the beginning of this work were equipped with \gls{BLE} technology. 
...
% dar uma introdução ao tema
% falar mais sobre os trabalhos
% diluir no capitulo motion sensors -> 2.2.2.5 scientific research using imu/cenários de aplicação
...
% Carl Fischer, Poorna Sukumar and Mike Hazas propose an approach to implement a tracker using a Kalman Filter and correcting the velocity using zero-velocity updates~\cite{tutorial}.
