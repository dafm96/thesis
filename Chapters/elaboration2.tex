...
\begin{figure}[htbp]
    ...
    \caption[Piezo accelerometer sensor]{Piezo accelerometer sensor~\cite{accelerometerspark}}
    \label{fig:acc_piezo}
\end{figure}
...
The most adopted class of gyroscopes for consumer use is the \gls{MEMS} gyroscopes, due to their low cost and low performance (but with satisfactory results for the common use casess). The unit used to measure angular velocity is degrees per second (º/s). \gls{MEMS} gyroscopes usually use a vibrating mechanical element to determine the rate of rotation, in 1, 2 or 3 axes~\cite{gyrospark}.
...
\begin{figure}[htbp]
    ...
    \caption[Gyroscope sensor]{Gyroscope sensor~\cite{gyromems}}
    \label{fig:gyro}
\end{figure}
...
This effect can be used to measure the Magnetic North of the Earth, providing handheld devices a sense of orientation in relation to Earth. However, these magnetometer measurements are easily disturbed by nearby objects, like a computer or a Wi-Fi router, which makes these handheld devices unreliable~\cite{magnetometer1}~\cite{magnetometer2}. For better results (not affected by other electromagnetic fields), industry or militar grade magnetometers should be used.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{magnetometer}
    \caption[Magnetometer sensor]{Magnetometer sensor~\cite{magnetometer1}}
    \label{fig:magnetometer}
\end{figure}

\subsubsection{GNSS}
\label{subsubsec:gnss}
\gls{GNSS} stands for Global Navigation Satellite System, which is a system that provides autonomous geo-spatial positioning, and allows small electronic devices to determine their location precisely. This location is expressed in longitude, latitude and altitude.
...
\begin{figure}[htbp]
    ...
    \caption[GNSS System]{GNSS System~\cite{gnssimg}}
    \label{fig:gnss}
\end{figure}

\subsubsection{Video Tracking Systems}
\label{subsec:video_approach}

Camera systems are widely used in sports and can accurately track the players of both teams and the ball. One example of this system is Stats SportVU, which is used in various sports, like soccer~\cite{stats_football} and basketball~\cite{stats_basketball}. By analyzing the video using machine learning techniques, this system can provide statistics like distance traveled, average and maximum speeds, number of speeds, field coverage, team formation and many others. A representation of this system is shown in Figure~\ref{fig:cameras}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\linewidth]{cameras}
  \caption[SportVU cameras in a basketball court]{SportVU cameras in a basketball court~\cite{stats_basketball}}
  \label{fig:cameras}
\end{figure}

Second Spectrum, the official optical tracking provider for \gls{NBA}~\cite{secondspectrum}, uses cameras located around the court to collect 3D spatial data of players and the ball like position and movements, and to gather data like speed, distance, shoots, passes, drives and many more ~\cite{secondspectrumaws}. Then, using machine learning algorithms, players and coaches can get detailed reports and insights about player performance. This approach can also be profitable for leagues and media, providing data to interactive applications or using augmented reality video to improve the fan experience.

In Football, video tracking systems are used in the Video Assistant Referees system used by \gls{FIFA}~\cite{fifavar}, which reviews the decisions made by the referee using video footage of the game. A video system is also used for the Goal-line Technology, to detect if the whole of the ball has crossed the goal line~\cite{fifagoalline}.
...
Wi-Fi technology is based on the \gls{IEEE} wireless communication standard 802.11, which has been around for over 20 years, with various different versions of the protocol.
...
Bluetooth is managed by Bluetooth \gls{SIG}, which develops, licenses and holds the trademark. To be able to use Bluetooth, a device manufacturer must meet the Bluetooth \gls{SIG} standards~\cite{blesig}.
...
\begin{figure}[htbp]
    ...
    \caption[BLE Topologies]{BLE Topologies~\cite{ble_mesh_topologies}}
...
In order to meet the objective, a low power and low-cost device is needed, with a medium range (enough to cover a basketball court), and a data throughput capable of handling rates of 20-50 Hz (this interval is enough to ensure that every step has about 10 to 20 samples of data, considering that the pedestrian walking stride frequency is about 1 Hz, and a the running stride frequency between 2 and 3 Hz).
%TODO Como é que se consegue replicar este trabalho? (GIT) não aqui?
...
In order to build the system prototype, a system architecture must be defined first. Due to the different characteristics of \gls{IoT} devices, in comparison to computer, like the reduced size, complexity, storage and connectivity limitations, the way these systems are organized can be changed and improved. This subsection presents different types of architectures, to better chose the one to use in the developed system.
...
The advantage of this architectural change frees organizations to keep server infrastructures on site, and allows them to have more resources than they could have on premises. It also allows the collection of data from multiple locations and devices, using the internet as a mean of transmitting all the data. The cloud is also capable of performing high performance computing, such as predictive analysis, and have access to a huge amount of data, as the data is also centralized in the cloud~\cite{Inc}. This could imply a higher latency in computation time, due to network and data transfer delays, but this approach brings the benefit of having more computational power, and the detachment of a fixed server infrastructure.
...
Fog computing reduces the involvement of the cloud and brings the computation closer to the sensing devices, while still allowing the geographical distribution of these sensing devices, improving the use of computation power and reducing the processing time. The fog layer is composed of multiple fog nodes, that receive requests from the \gls{IoT} devices or the end users. 
...
~J.Sampaio~\textit{et al.}~\cite{Sampaio2015} analyze the game performance of \gls{NBA} players, from open-access \gls{NBA} data collected with the SportsVU system, and grouped some variables, to obtain information about game actions, like pull-up shots, catch and shoots, close shots, drives, passing-variables (like scores from passes, or assists), touches-variables (like touches per game or time of possession), speed and distance, rebounds and free-throw percentage.
With his work made an analysis of a \gls{NBA} season, using player tracking variables. These variables are collected in every \gls{NBA} game, and are important to understand and analyze the players performance.

Marco Mangiarotti~\textit{et al.}~\cite{Mangiarotti2019} developed a system that is capable of identifying basketball activities, using two wrist-worn devices, with built-in accelerometers and gyroscopes. By using two machine learning algorithms in parallel (K-Nearest Neighbours and Support Vector Machine), they only recognize an activity if both algorithms identify it, reducing the probability of misclassification. They recognized three activities: shoots, passes and dribbles, but did not evaluate the performance of the dribble recognition. The recognition of the other two movements resulted in a very positive result, with a precision of 99\%.
...
Alexander Holzemann \textit{et al.}~\cite{Holzemann2018} has a similar approach, with wrist-worn \gls{IMU} Sensors, identifying activities such as dribbling, shooting, blocking, or passing. Using classifiers, a performance of 84\% is achieved with k-Nearest Neighbor classifier and 88\% with Random Forest classifier, with actions like jump shots performing better than dribbles.
%TODO Qual o melhor método de apresentar estas comparações?
%Pensou em fazer uma tabela comparativa com todos os sistemas?

\section{Commercial Solutions}
The most used way of track Basketball players is using optical tracking systems. Currently, Second Spectrum is the official optical tracking provider for the \gls{NBA}~\cite{NBAstuffer}. This system uses cameras located around the court to collect 3d spatial data of player and ball location and movements, and play-tracking data like speed, distance, shoots, passes, drives and team metrics like atack or defense tactics~\cite{AWSMe}.
...
SportVU is an optical tracking system, used by Stats Perform~\cite{StatsPerform}, that with cameras placed around the court and the use of artificial intelligence techniques can gather innovative statistics based on player speed, distance traveled, separation, ball possession and more. They combine the tracking of the player and the ball with historical databases to provide in-depth insights for team and player performances. This optical system is available for basketball and soccer, and besides providing useful information for team management, this data is also used for fan media, and for betting and fantasy content.

Many other sports use optical tracking systems in their games, and they can give very accurate measurements about the players location and team tactics, and even track small movements like shoots and passes in Basketball. However, these systems require high processing power, and are very expensive, being out of reach for minor and amateur teams. Optical systems are also attached to the field, and in order to track a team in a different location, the system needs to be installed there. Another drawback is that some optical systems don’t work in real-time. They need an operator to tag events, and the processed data is deferred~\cite{VanderKruk2018}.
...
STATSports~\cite{STATSports} is a system developed for soccer player tracking. The players use a pod worn in the back, that is equipped with an accelerometer, a gyroscope, a magnetometer and that collects positional tracking using a GPS device. This device has the ability to calculate internally over 50 metrics in real-time (\textit{e.g.} total distance covered, number of accelerations or decelerations and heart rate), that are then transmitted over a \gls{UWB} network. A cloud-based platform aggregates all the data, and presents available advanced metrics dashboards and individual/group data for players and coaches to consult. It is used widely by several professional soccer teams.

\section{Chapter Summary}
In this chapter we started by analyzing the commonly used motion sensors, the most suitable communication protocols to transmit information in this type of applications and what \gls{IoT} architectures exist, and the pros and cons of each one. With this analysis, it was concluded that 6LoWPAN, Wi-Fi and \gls{BLE} are the most suitable communication protocols to use, but only Wi-Fi and \gls{BLE} will be used in the implementation phase, as they are the only ones available.

By analyzing different \gls{IoT} computing architectures, it was also found that this kind of system could benefit from using an edge-processing style architecture, because it can improve the need to process data in real-time, allowing for low latency in the analysis of data.

Regarding the related works in this field, the search focused on works that studied human activity recognition in general, identifying activities like sitting, walking, running, jumps and tracking of position, and on works that aimed to classify basketball related activities, like shots, passes and dribbles, and what variables collected from a basketball game are used to evaluate the players performance. A similarity found between all the works is that the use of \gls{IMU} Sensors is a common practice, and placing them in the body parts where the movement occurs (\textit{e.g} in the hand to detect dribbles, in the foot to detect steps) results in good sets of data to train the recognition models. 

Most metrics are not only interested in isolated movements, like how many shoots or passes were made, but are interested in where those actions were made, and the context of the action. For example, was the shoot made inside or outside the three point line? Did pass resulted in a point? How long has the player been dribbling? And how far did he dribbled? To match these metrics, activities like dribbling, shoots and passes should be identified, but also the position and the path of the players should be tracked, to understand and contextualize the actions they are performing.

Of the researched works, all of the works found that aimed to classify activity (both human and basketball activities), used machine learning techniques to do so. These techniques, while powerful when trained with large amounts of data, are often used to perform the classification offline, after collecting the data. They are also very resource-intensive. These factors are against the objectives defined for this study. While they are useful to understand what activities and metrics can be classified, other techniques (such as signal analysis and statistics) should be used in the implementation of this system. These methods could be incorporated in the developed system in the future, to provide more complex analysis, with more data, processed after the games and practices.

In the area of positional tracking, three promising works were found. All revolve around the same concepts: use \gls{IMU} to detect steps, displacement and orientation. The work of Wonho Kang and Youngnam Han~\cite{Kang2015} uses the data collected from a smartphone, with data collected when the smartphone is in the hand, in front of the subject. For basketball players, this is an impossible scenario. The other two works, both used a sensor placed in the foot, which seems like a non intrusive method for collecting data from basketball players. Comparing the two, the work of Carl Fischer, Poorna Sukumar and Mike Hazas~\cite{Fischer2013} seems to produce better overall results, and their work is more detailed. This work will be used later in the implementation of the system prototype.

Regarding the commercial solutions, optical-based and sensor-based approaches were analysed. Optical systems are discarded, since they don't meet the objectives of this study in terms of cost, performance power and infrastructure. Therefore, that leads us to build a system based on wearable sensors, and so those systems are analysed. 
...
During the development of this work, two companies were contacted. Although we do not have a scenario for it to be possible to calculate a budget, the responses were that the cost of the systems could range between tens to hundreds of thousands of euros, by the time of writing of this dissertation.
