...
\epigraph{This chapter presents the implementation of the system proposed in Chapter~\ref{cha:systemProposal}. It starts by describing the data collected from the players. Afterwards, the performance metrics are explained, and finally the details of the implementation are illustrated. The code implemented on the server can be consulted in \url{https://github.com/dafm96/BLE_SportsTracker}, and the code implemented in the Rasberry Pi's can be consulted in \url{https://github.com/dafm96/BLE_SportsTracker_rPi}. }
...
\begin{figure}
    \centering
    \includegraphics[width=.5\textwidth]{tutorial_methodology.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.3\textwidth]{jump.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.5\textwidth]{jumpSteps.pdf}
...
In terms of cost, each one of the \gls{IMU} sensors used costs about 50€, and a Raspberry Pi 4 can be bought for about 40€. For a minimal team setup, which means equipping each player in the court (5 players) with 3 sensors, a total of 15 sensors is needed. To collect data from 15 sensors, a minimal of 2 Raspberry Pi are needed. All summed, a minimal setup has a cost of 830€. This price only accounts for the hardware, leaving development and maintenance costs out. However, when compared to other systems, it has a much lower price.
...
\paragraph{\textbf{Teams}}~\\
\indent The Teams endpoint enables the \gls{CRUD} actions over the teams of the system.
...
\paragraph{\textbf{Players}}~\\
\indent The Players endpoint provides the user the \gls{CRUD} actions over the players of the system.
...
\paragraph{\textbf{Games}}~\\
\indent The Games endpoint, besides providing Create, Read and Delete operations for the games, also manages the attribution of peripherals to each player, in multiple parts of their body, and also to fetch the metrics for each player.
...
\paragraph{\textbf{Peripherals}}~\\
\indent The Peripherals endpoint enables the control of the \gls{IMU} Sensors from the Client app, and stores temporary state on the peripherals, like what are the currently connected peripherals, and which ones are collecting data.
...
The implementation of the client application was developed with a focus on the coach perspective. In order to protect the data from other teams, a coach should only be able to edit and view the information about their team. At this development phase, this data protection features weren't implemented. Some of the API features described in Subsection \ref{susubsec:API} weren't implemented in the client application, like the ability to edit and delete teams and players.
...
To illustrate the usage of the developed prototype, let's set a practice game during a training session as the scenario. The Raspberry Pi's and the server should be running.
