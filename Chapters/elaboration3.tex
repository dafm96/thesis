...
It should be able to work indoors, thus avoiding the use of systems like \gls{GPS}, and it should work in any field, without the need of pre-installed structures (like a video system mounted in the court, or a set of antennas placed around the court).

It also should provide real-time data (no more than a few seconds, from when the movement was performed) of individual game metrics, calculated with the raw data from wearable sensors, like the identification of jumps, passes, dribbles, shoots and the player position. These game metrics allow coaches to improve their player and team performance during training sessions and games, injury prevention and recovery. The purpose of the raw data is to calculate more advanced performance metrics, and should be discarded after being used, so it doesn't waste storage space.

A game of basketball is played by two teams of 5 players, and 8 players at the most can be on the bench, to substitute the ones in court. If every player uses 3 devices, this could mean data collection from up to 30 devices to track every player in court during a basketball game.
...
\begin{itemize}
    ...
    \item Provide real-time game metrics of game performance
...
%TODO Qual o custo da solução?
%E em termos de manutenção? Qualquer pessoa duma área de desporto tem as competências necessárias?
...
%TODO Muitos sensores não vão implicar muitas ligações Wi-Fi (ou semelhantes)? a largura de banda não vai ficar saturada?
...
The Server also hosts the Client Application, which communicates with the Server. For this communication, an \gls{API} should be available, to expose system functions to external applications, such as the Client application, or other applications that may use the data collected and stored in the Server.

\subsection{Client}
\label{subsec:client}
The Client Application provides the end user an interface in which he can control and consult information about players, teams and games. This interface should work seamlessly in multiple platforms, providing a similar user experience no matter the device being used. The preferred method to use the application should be a native mobile app, even though a multi-device browser application could also be suitable.

The application interface should offer different operations and views for different types of users. Fans could consult more generic metrics and statistics, while a coach could manage their team and players, see detailed metrics and statistics of a player, a team or a match, and manage the sensors being used by the players.

The application should, indirectly, trigger the start and stop of the data collection in the \gls{IMU} Sensors, which will then send the raw data to the \gls{EPU}. After the game metrics are calculated in the Edge, they are shown to the user, in the Client Interface. Only the coaches should be allowed the management of the data collection. Fans using the application should only be allowed to view the processed game metrics.

\section{System Proposal Summary}

In this chapter a set of requirements were set, in order to build a system prototype that matches the objectives set in Section~\ref{sec:objectives}.
These requirements define that the system must be able to work indoors, without the need of pre-installed infrastructures.
It should provide real-time game metrics collected from all the players, and through an application coaches can manage their teams and fans can have access to statistics of the games.

A system architecture was also defined, dividing the system in three layers, following an edge-computing system architecture.
The edge layer should be composed of wearable sensors, that collect data from the players, and \gls{EPU}, that that receive this information and process it, calculating game metrics. These processed data is sent to the server, which stores it and can perform a more in-depth analysis.
The data is then presented to the end-user (coaches or fans), providing insights and statistical analysis about the game.

The next chapter will transform this set of requirements and system architecture description into a system prototype, explaining all the development processes.